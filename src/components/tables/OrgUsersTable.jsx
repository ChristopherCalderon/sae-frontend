    ...
    <div className="p-4 font-mono h-full">
      <div className="max-h-full overflow-y-auto overflow-x-auto border border-gray-300 rounded-md shadow-md">
        <table className="w-full text-sm table-auto border-collapse">
          ...
          <tbody>
            {users.map((u, i) => (
              <tr key={i} className="border-t border-gray-300 bg-white text-center">
                <td className="px-2 py-2 border border-gray-300">{u.name}</td>
                <td className="px-2 py-2 border border-gray-300">
                  {u.organizations[0].role}
                </td>
                <td className="px-2 py-2 border border-gray-300">
                  {u.organizations[0].role !== "ORG_Admin" && (
                    <button
                      onClick={() =>
                        handleStatusChange(
                          orgId,
                          u._id,
                          !u.organizations[0].isActive
                        )
                      }
                      className="bg-secondary hover:bg-primary hover:text-white text-primary font-bold text-xs px-3 py-1 rounded"
                    >
                      {u.organizations[0].isActive ? "Activo" : "Inhabilitado"}
                    </button>
                  )}
                </td>

                <td className="px-2 py-2 border border-gray-300">
                  {u.organizations[0].role === "Teacher" &&
                    u.organizations[0].isActive && (
                      <button
                        onClick={() => handleAdminChange(orgId, u._id)}
                        className="bg-secondary hover:bg-primary hover:text-white text-primary font-bold text-xs px-3 py-1 rounded"
                      >
                        Asignar
                      </button>
